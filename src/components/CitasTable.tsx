...
/**
 * CitasTable — tabla premium de gestión de citas para el dashboard del médico.
 *
 * Recibe el listado inicial ya resuelto por el server component que la
 * incrusta (filtrado por `id_medico`) y gestiona las acciones de "Confirmar"
 * / "Cancelar" contra `POST /api/dashboard/actualizar-estatus`:
 *  - Mientras la petición está en curso, la fila se atenúa (`opacity`) y sus
 *    botones se reemplazan por un spinner — no hay doble clic posible.
 *  - Al recibir la cita actualizada del servidor, el badge de estatus se
 *    refresca de inmediato en memoria (sin recargar la página).
 *  - Un fallo de red o de la API se muestra en un banner, sin descartar el
 *    resto de la tabla.
 */
export function CitasTable({ citasIniciales }: CitasTableProps) {
  ...
  const actualizarEstatus = useCallback(async (idCita: string, nuevoEstatus: EstatusCita) => {
    setErrorGlobal(null);
    ...
    try {
      ...
      console.error('[CitasTable] Error de red al actualizar la cita:', error);
              ...
              return (
                <tr
                  ...
                  className={[
                    ...
                    enProceso ? 'opacity-50' : 'opacity-100 hover:bg-slate-50/60',
                  ].join(' ')}
                >
